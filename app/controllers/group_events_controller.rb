class GroupEventsController < ApplicationController
  ...
  # POST /group_events/1/publish
  # POST /group_events/1/publish.json
  def publish
    @group_event_validator = GroupEventValidator.new(@group_event.attributes)

    respond_to do |format|
      if @group_event_validator.valid? && @group_event.publish!
        format.html { redirect_to @group_event, notice: 'Group event was successfully updated.' }
        format.json { render :show, status: :ok, location: @group_event }
      else
        format.html { render :edit }
        format.json { render json: @group_event_validator.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  def destroy
    @group_event.disable!
    respond_to do |format|
      format.html { redirect_to group_events_url, notice: 'Group event was successfully disabled.' }
  ...
  private
    ...
    def group_event_params
      params.require(:group_event).permit(:user_id, :starts, :duration, :name, :description, :location)
    end
end
